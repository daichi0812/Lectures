...
#define CAMERA_TIME_STEP 100 // カメラ画像の更新周期 [ms]
...
int main(int argc, char **argv)
{
  ...
  while (wb_robot_step(TIME_STEP) != -1)
  {
    ...
    for (i = 0; i < 10; i++) // すべてのLEDについて繰り返す
    {
      if (((int)current_time / 8) % 8 == i)
      {
        led_value[i] = true;
      }

      if ((int)current_time % 8 == i)
      {
        led_value[i] = true;
      }
      
      if(((int)current_time / 8) % 8 != i && (int)current_time % 8 != i){
        led_value[i] = false;
      }

      if ((int)current_time % 8 == i && ((int)current_time / 8) % 8 == i)
      {
        led_value[i] = false;
      }
    }
