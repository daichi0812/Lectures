 ...
 * File: e-puck用コントローラ例・2-5
 * Date: 2024/4/30
 * Description: キャリブレーション終了後、ロボットが前進を開始し、前方のセンサに障害物が接近したら停止する
 */
...
int main(int argc, char **argv)
{
  ...
  while (wb_robot_step(TIME_STEP) != -1)
  {
    ...
    { // 現在時刻が10秒未満のとき
      ...
      // キャリブレーション中は実測値を表示
      speed[LEFT] = 0 * MAX_SPEED; // キャリブレーション中は停止
    ...
    {                       // 現在時刻が10秒以上のとき
      ...
      if (nrm_dist_value[0] > 300 || nrm_dist_value[7] > 300)
      {
        // 正面に壁があったら
        speed[LEFT] = 1.0 * MAX_SPEED; // 右に急旋回
        speed[RIGHT] = -1.0 * MAX_SPEED;
      }
      else if (nrm_dist_value[6] > 300) // 左斜め前に近めに壁があったら
      {
        speed[LEFT] = 0.9 * MAX_SPEED; // 右に早く曲がりながら前進
        speed[RIGHT] = 0.2 * MAX_SPEED;
      }
      else if (nrm_dist_value[6] > 200) // 左斜め前に少し近めに壁があったら
      {
        speed[LEFT] = 0.8 * MAX_SPEED; // 右に少し早く曲がりながら前進
        speed[RIGHT] = 0.3 * MAX_SPEED;
      }
      else if(nrm_dist_value[6] > 100)
      {
        speed[LEFT] = 0.7 * MAX_SPEED; // 右に曲がりながら前進
        speed[RIGHT] = 0.4 * MAX_SPEED;
      }
      else if (nrm_dist_value[6] > 100) // 左側真横に壁があったら
      {
        speed[LEFT] = 0.6 * MAX_SPEED; //  緩やかに右に曲がりながら前進
        speed[RIGHT] = 0.5 * MAX_SPEED;
      }
      else
      {                                // 正面にも左にも壁がないとき
        speed[LEFT] = 0.34 * MAX_SPEED; // 左に早めに曲がりながら前進
        speed[RIGHT] = 1.0 * MAX_SPEED;
      }
    }
