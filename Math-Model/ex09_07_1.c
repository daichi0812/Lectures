/*
 *ex09_07_1.c 15822108　情報テクノロジー 堀田大智
 */
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
int main(void)
{
  ...
  /* ファイルのオープン */
  if ( (fin = fopen( "ex09_07_1_input.dat", "r")) == NULL )
  {
      printf("ファイルが見つかりません : ex09_07_1_input.dat \n");
      exit(1);
  }
  if( (fout = fopen( "ex09_07_1_output.dat", "w")) == NULL )
  {
    printf("ファイルが作成できません : ex09_07_1_output.dat \n");
    exit(1);
   }
...
double *jacobi_lin(double **a, double *b, double *x, int N)
{
  ...
  if((fout=fopen("ex09_07_1.csv", "w"))==NULL){
	  printf("ファイルが作成できません: ex09_07_1.csv\n");
	  exit (1);
  }
