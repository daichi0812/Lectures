/*
 *ex09_07_2.c 15822108 情報テクノロジー　堀田大智
 */
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
/* ガウス・ザイデル法 */
double *gauss_seidel(double **a, double *b, double *x, int N);
...
int main(void)
{
  ...
  /* ファイルのオープン */
  if ( (fin = fopen( "ex09_07_2_input.dat", "r")) == NULL )
  {
      printf("ファイルが見つかりません : ex09_07_2_input.dat \n");
      exit(1);
  }
  if( (fout = fopen( "ex09_07_2_output", "w")) == NULL )
  {
    printf("ファイルが作成できません : ex09_07_2_output \n");
    exit(1);
   }
  ...
  x = gauss_seidel( a, b, x, N );          /* ガウス・ザイデル法 */
...
double *gauss_seidel(double **a, double *b, double *x, int N)
{
  ...
  if((fout=fopen("ex09_07_2.csv", "w"))==NULL){
	  printf("ファイルが作成できません: ex09_07_2.csv");
	  exit (1);
  }
