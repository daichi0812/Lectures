...
void initState( System& io_System )
{
  ...
  for( int i=4; i<io_System.num_bodies; i++ )
  {
	...
	state.q.segment<3>(3 * i) << (randomMT() * 2.0 - 1.0) * (1.0 - state.r(i)), (randomMT() * 2.0 - 1.0) * (1.0 - state.r(i)), 0.0;//randomMT() * 2.0 * M_PI;
	state.v.segment<3>(3 * i) << (randomMT() * 2.0 - 1.0) * 0.01, (randomMT() * 2.0 - 1.0) * 0.01, 0.0;//(randomMT() * 2.0 - 1.0) * 10.0 * M_PI;
