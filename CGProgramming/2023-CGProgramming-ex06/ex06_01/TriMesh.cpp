...
{
  std::vector<Eigen::Matrix3i> triangles;
  // v0, v1, v2
  // vt0, vt1, vt2
  // vn0, vn1, vn2
...
bool loadObj( const std::string& in_filename, Object& out_object )
{
  ...
  for( int i=0; i<internal_triangles.size(); i++ )
  {
    ...
    {
      mesh.kd = internal_materials[internal_triangles[i].material_id].kd;
    
      if( internal_materials[internal_triangles[i].material_id].texture_name.length() > 0 )
      {
        mesh.texture = prepareTextureFromJpegFile( internal_materials[internal_triangles[i].material_id].texture_name.c_str() );
      }
      else
      {
        mesh.texture = 0;
