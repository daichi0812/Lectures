...
public class GraphPanel extends JPanel {
	...
	public GraphPanel(double x1, double x2, int division) {
		...
		x_i = new double[10];
		// 定義域内の適当な2点を分割法させる。
		x_i[0] = solver.solveSecant(-10.0, -9.9);
		for(j = (int)x1 + 1; j <= (int)x2; j += h2 ) {
			root = solver.solveSecant(-10.0, j);
			if(x1 <= root 
					&& root <= x2 
					&& Math.round(root * 1000.0) / 1000.0 != Math.round(x_i[count2 - 1] * 1000.0) / 1000.0
					&& Math.round(solver.f(root) * 1000.0) / 1000.0 == 0.000) {
				x_i[count2] = root;
				count2++;
			}
		}
